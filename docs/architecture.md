...
- **Deterministic validation pipeline** — LLM output is filtered before anything is written: predicates must come from the controlled vocabulary and confidence is clamped to [0,1] (both are hard rejections). *Grounding* is checked separately: at least one of subject/object should appear in the episode text (case-insensitive), match a configured owner alias (`ENGRAM_OWNER_ALIASES`; with built-ins `I`, `me`, `my`, `user`, `the user`), or resolve to an already-existing entity in the episode's group. At most 20 triples are stored per episode, preferring grounded triples first and then the highest confidence when the model returns more
- **Quarantine, not rejection, for ungrounded triples** — a triple grounded on neither side is still stored, with `grounded: false`, rather than discarded; `search_knowledge` excludes quarantined triples by default (`include_ungrounded` opts back in). A `recurrence` counter tracks corroboration: re-extracting the same triple (same subject/predicate/object/group) from a *different* source episode increments `recurrence` and raises `confidence` to the max of the two observations; a same-episode re-extraction is treated as re-run noise and does not bump it. If that later corroborating extraction is grounded, it promotes the stored row to `grounded: true` — independent evidence can rescue a quarantined fact
- **Recurring dreams surface for human review** — quarantined triples with `recurrence >= 3` appear in `find_loose_ends` and as a `recurring_dreams` count on `/api/v1/status`: ideas the dreamer keeps having that nothing has anchored. The `resolve_knowledge` tool records the user's verdict — confirm promotes to `grounded` + `verified` (the only path to `verified: true` for dreamer triples), reject sets `expired_at` (demoted, never deleted)
