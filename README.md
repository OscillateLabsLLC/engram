...
Quarantined triples that keep recurring (3+ distinct episodes) are surfaced as *recurring dreams* — in `find_loose_ends`, and as a count on `/api/v1/status`. They're an invitation to a conversation: an assistant can raise them with you, and record your verdict with `resolve_knowledge` (confirm promotes the fact to grounded and verified; reject expires it without deleting).
