...
// TestKnowledgeGraphTablesDropped verifies the knowledge-graph retirement
// migration removes the entities/knowledge/episode_links tables on startup.
func TestKnowledgeGraphTablesDropped(t *testing.T) {
	store := setupTestStore(t)
	defer store.Close()

	for _, table := range []string{"entities", "knowledge", "episode_links"} {
		var count int
		...
		if count != 0 {
			t.Errorf("Table %s should have been dropped during knowledge-graph retirement", table)
		}
	}
}
