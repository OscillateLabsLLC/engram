...
func TestListKnowledgeForReembed(t *testing.T) {
	store := setupTestStore(t)
	defer store.Close()
	ctx := context.Background()

	subject, err := store.InsertEntity(ctx, &models.Entity{CanonicalName: "Mike", Embedding: makeEmbedding(1), EmbeddingModel: "old-model"}, 0.88)
	if err != nil {
		t.Fatalf("Failed to insert subject: %v", err)
	}
	object, err := store.InsertEntity(ctx, &models.Entity{CanonicalName: "Engram", Embedding: makeEmbedding(1), EmbeddingModel: "old-model"}, 0.88)
	if err != nil {
	...
	t.Run("composes triple text from joined entity names", func(t *testing.T) {
		items, err := store.ListKnowledgeForReembed(ctx, "new-model", "", 10, false)
		if err != nil {
			t.Fatalf("ListKnowledgeForReembed failed: %v", err)
		}
		if len(items) != 1 {
			t.Fatalf("Expected 1 stale triple, got %d", len(items))
		}
		if items[0].Text != "Mike builds Engram" {
			t.Errorf("Expected triple text 'Mike builds Engram', got %q", items[0].Text)
		}
	})

	t.Run("entities are stale too", func(t *testing.T) {
		items, err := store.ListEntitiesForReembed(ctx, "new-model", "", 10, false)
		if err != nil {
			t.Fatalf("ListEntitiesForReembed failed: %v", err)
		}
		if len(items) != 2 {
			t.Errorf("Expected 2 stale entities, got %d", len(items))
		}
	})

	t.Run("update clears staleness across tables", func(t *testing.T) {
		if err := store.UpdateEntityEmbedding(ctx, subject.ID, makeEmbedding(2), "new-model"); err != nil {
			t.Fatalf("UpdateEntityEmbedding failed: %v", err)
		}
		if err := store.UpdateKnowledgeEmbedding(ctx, triple.ID, makeEmbedding(2), "new-model"); err != nil {
			t.Fatalf("UpdateKnowledgeEmbedding failed: %v", err)
		}
		counts, err := store.CountReembedTargets(ctx, "new-model", false)
		if err != nil {
			t.Fatalf("CountReembedTargets failed: %v", err)
