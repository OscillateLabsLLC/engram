...
echo '{"structured_output":{}}'`)
		...
		want := []string{"-p", "--output-format", "json", "--json-schema", `{"my":"schema"}`}
		...
		if !strings.Contains(string(stdin), "SYSTEM PROMPT") || !strings.Contains(string(stdin), "USER PROMPT") {
			t.Errorf("Stdin should contain system and user prompts, got %q", stdin)
		}
	})
